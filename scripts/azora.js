function decodeHTML(text){if(!text)return"";return text.replace(/&#([0-9]{1,4});/gi,function(m,n){return String.fromCharCode(parseInt(n,10))}).replace(/&#x([a-f0-9]+);/gi,function(m,h){return String.fromCharCode(parseInt(h,16))}).replace(/&amp;/g,"&").replace(/&lt;/g,"<").replace(/&gt;/g,">").replace(/&quot;/g,'"').replace(/&apos;/g,"'")}
function cleanHtml(text){return text.replace(/(<([^>]+)>)/gi,"").trim()}
...
function fetchMangaDetails(url){
    ...
    for(var i=1;i<chunks.length;i++){
        ...
        if(hrefMatch){
            ...
            var text="الفصل";
            if(spanMatch)text=spanMatch[1].replace(//g,"").replace(/(<([^>]+)>)/gi,"").trim();
            var exists=false;
