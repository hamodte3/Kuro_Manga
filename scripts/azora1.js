...
function fetchLatestManga(page, query){
    ...
    while((match = linkRegex.exec(html)) !== null){
        ...
        if(link.indexOf("/chapter-") !== -1 || link.indexOf("/chapter/") !== -1) {
            continue;
        }
        ...
        if(altMatch && altMatch[1] && altMatch[1].indexOf("صورة") === -1 && altMatch[1].indexOf("cover") === -1){
            title = altMatch[1];
        } else {
        ...
        if(title === "الحالة" || title === "مانهوا" || title === "" || title === "Unknown" || title.indexOf("الفصل") !== -1) {
            continue;
        }
    ...
    for(var i = 1; i < chunks.length; i++){
        ...
        if(hrefMatch){
            ...
            for(var j = 0; j < chapters.length; j++){
                if(chapters[j].chapterUrl === link){ exists = true; break; }
            }
